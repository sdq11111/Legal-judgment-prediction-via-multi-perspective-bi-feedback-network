...
def build_MPBFN_WCA():
    ...
    cnn4 = Conv2D(filters = int(config.hid_size / 4), kernel_size = (4, config.word_embed_size), data_format='channels_last', activation = 'elu')(cnninput)
    pooling4 = MaxPooling2D(pool_size=(config.total_num + 1 - 4, 1))(cnn4)
    pooling4 = Reshape((int(config.hid_size / 4),))(pooling4)
    cnn5 = Conv2D(filters = int(config.hid_size / 4), kernel_size = (5, config.word_embed_size), data_format='channels_last', activation = 'elu')(cnninput)
    pooling5 = MaxPooling2D(pool_size=(config.total_num + 1 - 5, 1))(cnn5)
